...
function tb_init(domChunk) {
  $(domChunk).click(function() {
	...
	var teaser = Drupal.settings.nodeInfo;
    var g = this.rel || false;
    tb_show(t,a,g,teaser);
    this.blur();
    return false;
  });
}

function tb_show(caption, url, imageGroup, teaser) { //function called when the user clicks on a thickbox link
  ...
  try {
    ...
    if (urlType == '.jpg' || urlType == '.jpeg' || urlType == '.png' || urlType == '.gif' || urlType == '.bmp') { //code to show images
      ...
      imgPreloader.onload = function() {
        ...
        TB_WIDTH = imageWidth < 320 ? 480 : imageWidth + 215;
        TB_HEIGHT = imageHeight + 60;
        var tease = teaser[caption].teaser;		
		var more = teaser[caption].more;
		var medium = teaser[caption].medium;
        $('#TB_window').append('<a href="" id="TB_ImageOff" title="' + settings.next_close + '"><img id="TB_Image" src="' + url + '" width="' + imageWidth + '" height="' + imageHeight + '" alt="' + caption + '" /></a><div id="TB_caption"><h4>' + caption + '</h4>' + tease + '<div id="TB_medium">' + medium + '</div><div id="TB_more">' + more + '</div></div><div id="TB_secondLine">' + TB_PrevHTML + TB_NextHTML + '</div><div id="TB_closeWindow"><span id="TB_count">' + TB_imageCount + '</span><span id="TB_close"><a href="#" id="TB_closeWindowButton" title="' + settings.close + '">' + settings.close + '</a> </span></div>');
        ...
        if (!(TB_PrevHTML === '')) {
          function goPrev() {
            ...
            tb_show(TB_PrevCaption, TB_PrevURL, imageGroup, teaser);
            return false;
          }
        ...
        if (!(TB_NextHTML === '')) {
          function goNext() {
            $('#TB_window').remove();
            $('body').append('<div id="TB_window"></div>');
            tb_show(TB_NextCaption, TB_NextURL, imageGroup, teaser);
            return false;
          }
